...
class EntropyChunking:
    ...
    def _compute_ppl(self, context: str, target: str) -> float:
        """Compute perplexity of target text given context using API"""
        if not target:
            return 0.0

        # Prepare the full text for the API call
        if context:
            full_text = context + "\n" + target
        else:
            full_text = target
        ...
        try:
            # Use the OpenAI client to get logprobs
            response = self.openai_client.completions.create(
                model=self.model_name,
                prompt=full_text,
                max_tokens=0,  # We don't want generation, just logprobs
                echo=True,  # Return the input text with logprobs
                logprobs=1  # Get log probabilities
            )

            # Calculate perplexity from log probabilities
            choices = response.choices
            if choices and hasattr(choices[0], 'logprobs'):
                logprobs = choices[0].logprobs.token_logprobs
                # Filter out None values (for BOS token sometimes)
                valid_logprobs = [lp for lp in logprobs[1:] if lp is not None]  # Skip first token
...
class LongCodeZip:
    def __init__(
        ...
        self.max_position_embeddings = getattr(self.openai_client.config, "max_position_embeddings", 4096)
        ...
        if rank_only:
            ...
            if not large_functions or lines_in_large_functions == 0:
                ...
                if not valid_scores or max(valid_scores) == min(valid_scores):
                    ...
                    normalized_scores = [
                        (s - min_score) / (max_score - min_score) if not math.isinf(s) and not math.isnan(s) else 0.0
                        for s in scores]  # Normalize to [0, 1], default 0 for invalid
                    ...
                    for norm_score in normalized_scores:
                        ...
                        biased_rate = global_rate_for_large * (1 + importance_beta * (
                                    norm_score - 0.5) * 2)  # Scale norm_score diff to [-beta, beta]
                    ...
                    actual_lines_kept = sum(
                        initial_rates[i] * large_functions[i]["line_count"] for i in range(len(large_functions)))
                    ...
                    if actual_lines_kept > 0 and abs(
                            actual_lines_kept - target_lines_for_large) > 1:  # Adjust if difference is significant
                        adjustment_factor = target_lines_for_large / actual_lines_kept
                        logger.debug(f"Adjusting rates by factor: {adjustment_factor:.4f}")
                        final_rates = [max(0.0, min(1.0, r * adjustment_factor)) for r in
                                       initial_rates]  # Adjust and clamp again
                    else:
            ...
            fine_grained_pbar = tqdm(enumerate(code_chunks), total=len(code_chunks), desc="Fine-Grained Compression",
                                     leave=False)
            for i, chunk in fine_grained_pbar:
                ...
                if i in selected_indices:
                    ...
                    chunk_line_count = len(
                        [chunk for chunk in chunk_lines if chunk.strip()])  # Count only non-empty for logic
                    ...
                    if chunk_line_count >= min_lines_for_fine_grained and individual_fine_ratio < 0.999:
                        ...
                        if fine_grained_importance_method == "conditional_ppl":
                            ...
                            if not query or not query.strip():
                                ...
                                if math.isinf(query_ppl_without_context):
                                    ...
                                    pbar_cond = tqdm(enumerate(chunk_lines), total=len(chunk_lines),
                                                     desc=f"Func {i} Block CondPPL", leave=False)
                            ...
                            with torch.no_grad():
                                pbar = tqdm(enumerate(chunk_lines), total=len(chunk_lines),
                                            desc="Block Contrastive PPL", leave=False)
                                for block_idx, block in pbar:
                                    ...
                                    regular_ppl = self._calculate_perplexity_for_contrastive(block,
                                                                                             condition_text=regular_ppl_condition)
                                    ...
                                    cond_ppl = self._calculate_perplexity_for_contrastive(block,
                                                                                          condition_text=cond_ppl_condition)
                        ...
                        if use_knapsack:
                            ...
                            for block_idx, (block, block_importance) in enumerate(zip(chunk_lines, block_importances)):
                                ...
                                for line_idx_in_block, line in enumerate(block_lines):
                                    ...
                                    line_importances.append(
                                        block_importance)  # Use block importance for all lines in block
                            ...
                            if valid_scores:
                                ...
                                if min_valid_score == float('inf') or min_valid_score == -float('inf') or math.isnan(
                                        min_valid_score):
    ...
    def split_code_by_functions(self, code: str, language: str = "python",
                                custom_separator: str = "# --CHUNK_SEPARATOR-- #") -> List[str]:
        """
    ...
    def _calculate_perplexity_for_contrastive(self, text: str, condition_text: str | None = None) -> float:
        """Helper to calculate perplexity of text, optionally conditioned on condition_text"""
        def _prompt_logprobs(prompt: str):
        ...
        try:
            if condition_text:
                ...
                logprobs = [t.logprob for t in tgt_tokens if getattr(t, "logprob", None) is not None]

            else:
                resp = _prompt_logprobs(text)
                prompt_tokens = getattr(resp.choices[0].logprobs, "prompt", None)
                if not prompt_tokens or len(prompt_tokens) < 2:
                    return float('inf')
                logprobs = [t.logprob for t in prompt_tokens[1:] if getattr(t, "logprob", None) is not None]

            if not logprobs:
                return float('inf')

            avg_lp = sum(logprobs) / len(logprobs)
            ppl = math.exp(-avg_lp)  # perplexity
            return ppl
...
if __name__ == "__main__":
    ...
    completion_ids = compressor.model.generate(**tokenized_prompt, max_new_tokens=128,
                                               pad_token_id=compressor.tokenizer.eos_token_id)
    # Decode only the generated part, skipping special tokens
    completion = compressor.tokenizer.decode(completion_ids[0][len(tokenized_prompt.input_ids[0]):],
    ...
    completion_lines = [line for line in completion.split("\n") if
                        line.strip() and not line.strip().startswith(("#", "//"))]  # Simple comment removal
    cleaned_completion = completion_lines[
        0] if completion_lines else completion  # Take first non-comment line or original if none found
